...
#batchID=$1
chr_num=$1
prephased_chr_haps=$(realpath $2) # Give it the prephased .haps file
...
mkdir ${output_dir}/chr$chr_num

chunk_file="${chr_chunks}/chr${chr_num}_chunk.txt"

> ${output_dir}/chr$chr_num/out-${chr_num}-all

cat $chunk_file | while read start end ; do 
	echo "Running chr${chr_num} ${start}-${end}..."
	output_file_name=out-chr${chr_num}-${start}-${end}
	$impute -m ${genmap}/genetic_map_chr${chr_num}_combined_b37.txt \
 				 ...
 				 -allow_large_regions \
 				 -seed 367946 \
 				 -o ${output_dir}/chr${chr_num}/$output_file_name #> ./chr${chr_num}/${output_file_name}.temp 2>&1
 	cat ${output_dir}/chr${chr_num}/$output_file_name >> ${output_dir}/chr$chr_num/out-${chr_num}-all
done
...
#			-allow_large_regions \
#			-seed 367946 \
#			-o $output_file_name > /dev/null 2>&1
